...
type Props = { channel: string; channelIcon: "#" | "@"; draftKey?: string; replyTo?: { author: string; body: string } | null; onCancelReply?: () => void; onSend: (message: string, files: File[]) => Promise<void>; onTyping: (typing: boolean) => void };

export function MessageComposer({ channel, channelIcon, draftKey, replyTo, onCancelReply, onSend, onTyping }: Props) {
  const [message, setMessage] = useState("");
  const draftStorageKey = draftKey ? `kivo_draft_${draftKey}` : null;
  ...
  // restore the unsent draft when switching conversations
  useEffect(() => {
    if (!draftStorageKey) return;
    try { setMessage(localStorage.getItem(draftStorageKey) ?? ""); } catch { setMessage(""); }
  }, [draftStorageKey]);
