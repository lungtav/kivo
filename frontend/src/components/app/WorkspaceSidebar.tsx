  ...
  return <aside className="flex h-full shrink-0 overflow-hidden bg-[#101014] text-stone-100">
    ...
    <div className={`overflow-hidden transition-[width,opacity] duration-200 ${isOpen ? "w-80 opacity-100" : "w-0 opacity-0"}`}>
      <div className="flex h-full w-80 flex-col bg-[#121217]">
        {settingsOpen ? <SettingsPanel onBack={() => setSettingsOpen(false)} /> : membersOpen ? <MembersPanel spaceId={space?.id ?? null} canManage={canManage} ownRole={space?.role ?? null} onBack={() => setMembersOpen(false)} onLeaveSpace={props.onLeaveSpace} onViewProfile={setProfileId} /> : view === "home" ? <>
...
function ProfileModal({ userId, onClose }: { userId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    void getProfile(userId)
      ...
      .catch((cause) => setError(cause instanceof Error ? cause.message : "Could not load the profile."));
  }, [userId]);
  return <Modal title="Profile" onClose={onClose}>{error && <p className="text-sm text-red-300">{error}</p>}{profile && <div className="flex items-center gap-3"><span className="grid size-12 shrink-0 place-items-center rounded-xl bg-violet-400 text-sm font-bold text-violet-950">{initials(profile.display_name)}</span><div className="min-w-0"><p className="truncate text-sm font-semibold text-stone-100">{profile.display_name}</p><p className="truncate text-xs text-stone-500">@{profile.username}</p>{profile.created_at && <p className="mt-1 text-xs text-stone-600">In Kivo since {new Date(profile.created_at).toLocaleDateString()}</p>}</div></div>}{!profile && !error && <p className="text-sm text-stone-400">Loading profile…</p>}</Modal>;
}
