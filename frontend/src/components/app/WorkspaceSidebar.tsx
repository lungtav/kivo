...
export function WorkspaceSidebar(props: Props) {
  ...
  // stale results are cleared per keystroke and late responses are ignored,
  // so the list never shows people matching an older query.
  useEffect(() => {
    ...
    const id = (peerSearchId.current += 1);
    const timer = window.setTimeout(() => {
      void searchUsers(query)
        .then(({ peers: items }) => { if (peerSearchId.current === id) setSearched(items); })
        .catch(() => { if (peerSearchId.current === id) setSearched([]); })
        .finally(() => { if (peerSearchId.current === id) setSearchingPeers(false); });
    }, 250);
  ...
  const createGroup = async () => {
    ...
    setMembersError(null);
    try {
      await props.onCreateGroup(groupName.trim(), groupMembers);
      setMemberPickerOpen(false);
      setGroupName('');
      setGroupMembers([]);
    } catch (cause) {
      setMembersError(cause instanceof Error ? cause.message : 'Could not create the group.');
    } finally {
      setBusy(false);
    }
  };
  ...
  return <aside className="relative flex h-full shrink-0 overflow-hidden bg-background text-foreground">
      ...
      <div className="flex h-full w-full flex-col bg-secondary">
        {settingsOpen ? <SettingsPanel onBack={() => setSettingsOpen(false)} /> : membersOpen ? <MembersPanel spaceId={space?.id ?? null} canManage={canManage} ownRole={space?.role ?? null} onBack={() => onMembersOpenChange(false)} onLeaveSpace={props.onLeaveSpace} onViewProfile={viewProfile} presence={presence} /> : view === "home" ? <>
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain px-3 py-5">
            {space === null && selectedSpaceId !== null && (
            ...
            {space === null && selectedSpaceId === null && (
              <div className="px-2 py-6 text-center">
                ...
                <p className="mt-1 text-xs leading-5 text-muted-foreground">Create one with the + button below, or join with an invite code.</p>
              </div>
            )}
    ...
    {memberPickerOpen && <Modal title={groupMode ? "New group" : "Start a direct message"} onClose={() => setMemberPickerOpen(false)}><div className="mb-2 grid grid-cols-2 gap-2"><button type="button" onClick={() => setGroupMode(false)} className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${!groupMode ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground hover:text-foreground"}`}>Direct</button><button type="button" onClick={() => setGroupMode(true)} className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${groupMode ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground hover:text-foreground"}`}>Group</button></div>{groupMode ? (<><input value={groupName} onChange={(event) => setGroupName(event.target.value)} placeholder="Group name" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:border-neutral-400 dark:focus:border-neutral-600" /><p className="mt-2 text-xs text-muted-foreground">Pick members — search by name or @username.</p></>) : (<input value={peerQuery} onChange={(event) => setPeerQuery(event.target.value)} placeholder="Search people by name or @username" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:border-neutral-400 dark:focus:border-neutral-600" />)} {membersError && <p className="mt-2 text-sm text-red-500">{membersError}</p>}{!isSearching && peers === null && !membersError && <p className="mt-2 text-sm text-muted-foreground">Loading people…</p>}{pickerRows !== null && pickerRows.length === 0 && !membersError && <p className="mt-2 text-sm text-muted-foreground">{isSearching ? "No one on Kivo matches that search." : "No conversations yet — search anyone by name or @username."}</p>}{isSearching && (searchingPeers || pickerRows === null) && <p className="mt-2 text-sm text-muted-foreground">Searching…</p>}<div className="mt-2 max-h-64 space-y-1 overflow-y-auto">{(pickerRows ?? []).map((peer) => groupMode ? (
            <button key={peer.id} onClick={() => toggleGroupMember(peer.id)} className={`flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left text-sm ${groupMembers.includes(peer.id) ? "bg-foreground/5 text-foreground" : "text-muted-foreground hover:bg-foreground/5"}`}>
              <span className={`grid size-4 shrink-0 place-items-center rounded border ${groupMembers.includes(peer.id) ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}>{groupMembers.includes(peer.id) ? "✓" : ""}</span>
              <span className="grid size-6 shrink-0 place-items-center rounded-full bg-primary text-[9px] font-bold text-primary-foreground">{initials(peer.display_name)}</span>
              <span className="min-w-0 truncate">{peer.display_name}<span className="ml-1.5 text-xs text-muted-foreground">@{peer.username}</span></span>
            </button>
          ) : (
            <button key={peer.id} onClick={() => void pickMember(peer.id)} disabled={busy} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left text-sm hover:bg-foreground/5 disabled:opacity-50">
              <span className="grid size-6 shrink-0 place-items-center rounded-full bg-primary text-[9px] font-bold text-primary-foreground">{initials(peer.display_name)}</span>
              <span className="min-w-0 truncate">{peer.display_name}<span className="ml-1.5 text-xs text-muted-foreground">@{peer.username}</span></span>
            </button>
          ))}</div>{groupMode && <div className="mt-3 flex justify-end"><button disabled={busy || !groupName.trim() || groupMembers.length === 0} onClick={() => void createGroup()} className="rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50">{busy ? "Creating…" : `Create group${groupMembers.length ? ` · ${groupMembers.length}` : ""}`}</button></div>}</Modal>}
    {joinOpen && <Modal title="Join a space" onClose={() => setJoinOpen(false)}><form onSubmit={submitJoin}><label className="block text-xs font-semibold text-foreground">Invite code</label><input autoFocus value={joinCode} onChange={(event) => setJoinCode(event.target.value)} placeholder="Invite code or link" className="mt-2 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:border-neutral-400 dark:focus:border-neutral-600" />{joinError && <p className="mt-2 text-xs text-red-500">{joinError}</p>}<div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setJoinOpen(false)} className="rounded-lg px-3 py-2 text-xs text-muted-foreground hover:bg-foreground/5">Cancel</button><button disabled={!joinCode.trim() || joinBusy} className="rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50">{joinBusy ? "Joining…" : "Join space"}</button></div></form></Modal>}
  </aside>;
}
...
function SettingsPanel({ onBack }: { onBack: () => void }) {
  ...
  const saveProfile = async (event: FormEvent) => {
    ...
    setError(null);
    try {
      const { user } = await updateProfile({ display_name: displayName.trim(), username: username.trim(), bio: bio.trim(), ...(avatarUrl ? { avatar_url: avatarUrl } : {}) });
      setProfile(user);
      setDisplayName(user.display_name);
      setUsername(user.username);
      setBio(user.bio ?? "");
