...
import { AtSign, ChevronLeft, Hash, MessageCircle, Phone, PhoneMissed, PhoneOff, Search, Settings, User, Users, Video, X } from "lucide-react";
...
function WorkspaceContent({ view, selectedSpace, selectedChannel, refreshConversations, onConversationActivity, onOpenMembers, onPresence, hasConversations, onOpenPalette, onBack }: { view: "home" | "space"; selectedSpace: Space | null; selectedChannel: SelectedConversation | null; refreshConversations?: () => void; onConversationActivity?: (conversationId: string, kind: "read" | "new") => void; onOpenMembers?: () => void; onPresence?: (userId: string, online: boolean) => void; hasConversations: boolean; onOpenPalette: () => void; onBack?: () => void }) {
              ...
              {[0, 1, 2, 3].map((index) => (
                <div key={index} className={`flex gap-3 ${index % 2 ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <Skeleton className={`h-10 rounded-2xl ${index % 2 ? "ml-auto w-2/5" : "w-3/5"}`} />
                  </div>
                </div>
              ))}
            </div>
